...
Button::Button(BUTTON_TYPE type, glm::vec2 position, std::string button_text, bool isMenu) //, MenuState * destination
{
	...
	// Button Art
	if (type == NORMAL) {
		//m_texNormal = new Image("file path to normal button", position);
		//m_texNormal->scale(glm::vec2(0.24f, 0.12f), false);
		//m_texHover = new Image("file path to button when mouse is over button", position);
		//m_texHover->scale(glm::vec2(0.24f, 0.12f), false);
		//m_texClick = new Image("file path to button when it is clicked", position);
